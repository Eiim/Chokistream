 ...
 * 
 */
public class ChirunoUDPThread extends Thread {
	 ...
	 */
	private byte[] packetBuffer = new byte[1448]; // (MTU)
	 ...
	 */
	ChirunoUDPThread(DSScreen screen, ColorMode colorMode, double topScale, double bottomScale, InterpolationMode intrp) throws SocketException {
		activeScreen = screen;
	...
	public void run() {
		while (!shouldDie.get()) {
			...
			try {
				...
				logger.log("Attempting to receive packet", LogLevel.VERBOSE);
				
				if (data[0] != 0)
				{
					logger.log("data[0] != 0 (:", LogLevel.VERBOSE);
				}
